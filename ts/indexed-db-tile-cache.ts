...
export class IndexedDbTileCache extends Emitter {
     ...
     * not stored already.
     */
    public getTileEntry(tileCoordinates: ITileCoordinates, downloadIfUnavaiable?: boolean): Promise<IIndexedDbTileCacheEntry> {
        const dbRequest: IDBOpenDBRequest = indexedDB.open(this.options.databaseName, this.options.databaseVersion);

        return new Promise((resolve, reject) => {
            dbRequest.addEventListener("success", (dbEvent: any) => {
                const database: IDBDatabase = dbEvent.target.result;
                const tx = database.transaction([this.options.objectStoreName]).objectStore(this.options.objectStoreName).get(this.createInternalTileUrl(tileCoordinates));
    ...
    public createInternalTileUrl(tileCoordinates: ITileCoordinates): string {
        return this.options.tileUrl.split(/{x}/).join(tileCoordinates.x.toString()).split(/{y}/).join(tileCoordinates.y.toString()).split(/{z}/).join(tileCoordinates.z.toString());
    }
    ...
    public createTileUrl(tileCoordinates: ITileCoordinates): string {
        const randomSubDomain: string = this.options.tileUrlSubDomains[Math.floor(Math.random() * this.options.tileUrlSubDomains.length)];
    ...
    public downloadTile(tileCoordinates: ITileCoordinates): Promise<IIndexedDbTileCacheEntry> {
        return new Promise((resolve, reject) => {
            fetch(this.createTileUrl(tileCoordinates)).then(async (response) => {
                ...
                dbRequest.addEventListener("success", (dbEvent: any) => {
                    const database: IDBDatabase = dbEvent.target.result;
                    const tx = database.transaction([this.options.objectStoreName], "readwrite").objectStore(this.options.objectStoreName).put(tileCacheEntry);
    ...
    public purgeStore(): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            dbRequest.addEventListener("success", (dbEvent: any) => {
                const database: IDBDatabase = dbEvent.target.result;
                const tx = database.transaction([this.options.objectStoreName], "readwrite").objectStore(this.options.objectStoreName).clear();
